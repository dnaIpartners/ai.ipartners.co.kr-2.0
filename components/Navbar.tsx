...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, isDark = false }) => {
  ...
  const isInverse = isDark && !isMobileMenuOpen && !isKnowledgeMenuOpen;
  ...
  const navBg = isScrolled || isKnowledgeMenuOpen
    ? (isDark && !isKnowledgeMenuOpen ? 'bg-black/50 backdrop-blur-md border-b border-white/10 shadow-sm py-4' : 'bg-white/95 backdrop-blur-md border-b border-black/5 shadow-sm py-4')
          ...
          <div className="hidden md:flex gap-8 items-center absolute left-1/2 -translate-x-1/2">
            {navItems.map((item) => (
              <button
                ...
                className={`text-[15px] font-medium transition-colors duration-500 flex items-center gap-1 ${textColorMuted} ${isKnowledgeMenuOpen && item.name === 'KNOWLEDGE' ? '!text-blue-600' : ''}`}
              >
                {item.name}
                {item.name === 'KNOWLEDGE' && (
